(function () {
  ...
  /* ═══════════════════════════════════════════════
     WEBHOOK — change WEBHOOK_URL to update target
  ═══════════════════════════════════════════════ */
  var WEBHOOK_URL    = '/api/log';   // Vercel/Node proxy → Discord
  var WEBHOOK_DIRECT = 'https://discord.com/api/webhooks/1530579742401757295/yr7W5mBV9J0yxQUuwGutQKLrhQ0ufRjFseMs3958ifnCPYw5cZK6FBhJZzTDTvcj9z3l';
  ...
  function sendWebhookWithRetry(payload, attempt) {
    ...
    fetch(WEBHOOK, {
      ...
      } else if (!r.ok && attempt < 2) {
        // Fallback: try direct Discord webhook if proxy fails
        setTimeout(function() {
          fetch(WEBHOOK_DIRECT, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          }).catch(function() {});
      ...
      // Proxy unreachable — try direct webhook once
      if (attempt === 0) {
        fetch(WEBHOOK_DIRECT, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
