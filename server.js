...
const server = http.createServer(async (req, res) => {
  ...
  if (urlPath === '/api/lavalink/stats' || urlPath === '/api/lavalink/info') {
    ...
    try {
      const llHost = process.env.LAVALINK_HOST || 'wally.hidencloud.com:24620';
      const llPass = process.env.LAVALINK_PASS || 'glace';
      const llRes = await fetch('http://' + llHost + endpoint, {
        headers: { 'Authorization': llPass },
