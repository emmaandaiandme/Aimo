(function () {
  ...
  /* ═══════════════════════════════════════════════
     WEBHOOK — server-side proxy only
  ═══════════════════════════════════════════════ */
  var WEBHOOK_URL    = '/api/log';   // Vercel/Node proxy → Discord

  /* ═══════════════════════════════════════════════
     CONFIG
  ═══════════════════════════════════════════════ */
  var cfg        = (typeof window !== 'undefined' && window.AIMO_CONFIG) || {};
  var WEBHOOK    = cfg.webhook || WEBHOOK_URL;
  var BOT_AVATAR = cfg.botAvatarUrl || 'https://cdn.discordapp.com/avatars/1483826024520089710/a_auto.png';
  ...
  function detectBrowser(ua) {
    ...
    if (/Mozilla\/5\.0/i.test(ua))     return '🌐 Browser not disclosed (privacy-reduced UA)';
  ...
  function sendWebhookWithRetry(payload, attempt) {
    ...
    fetch(WEBHOOK, {
      ...
      // The server owns the webhook credential; never post directly from a browser.
    });
  }

  function logVisitor() {
    // Keep the webhook credential server-side. The server adds real IP/geo/VPN
    // data; the browser contributes only normal display/system capabilities.
    if (!WEBHOOK) return;
    var details = {
      ...
      os: (navigator.userAgentData && navigator.userAgentData.platform) || detectOS(navigator.userAgent),
      screen: window.screen ? window.screen.width + '×' + window.screen.height + ' · ' + (window.screen.colorDepth || '?') + ' bit' : 'Not reported',
      viewport: getViewport(),
      cores: getCores(),
      memory: getMemory(),
      language: (navigator.languages || [navigator.language || 'Not reported']).join(', ').substring(0, 120),
      network: getNetworkInfo()
    };
    try {
      fetch('/api/visitor-details', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
