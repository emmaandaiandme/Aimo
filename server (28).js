...
const { getLavalinkConfig } = require('./api/lavalink-config');
const { requestJson: requestLavalinkJson } = require('./api/lavalink-client');
...
const server = http.createServer(async (req, res) => {
  ...
  if (urlPath === '/api/lavalink/stats' || urlPath === '/api/lavalink/info') {
    ...
    const lavalink = getLavalinkConfig();
    const endpoint = urlPath === '/api/lavalink/stats'
      ? lavalink.statsPath
      : lavalink.infoPath;
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Cache-Control', 'no-cache');
    ...
    try {
      const data = await requestLavalinkJson(
        lavalink.baseUrl + endpoint,
        lavalink.password
      );
      res.writeHead(200, { 'Content-Type': 'application/json' });
      ...
      return res.end(JSON.stringify({
        error: e.statusCode ? e.message : 'Lavalink request failed',
        code: e.statusCode ? undefined : (e.code || 'UPSTREAM_UNREACHABLE')
      }));
    }
  }
